...
InteractableImageObject player1;
InteractableImageObject player2;
...
void setup() {
  ...
  //LOAD IMAGES TO USE FOR PLAYERS
  player1img = loadImage("data/beardo.png");
  player2img = loadImage("data/brewster.png");
  //SET IMAGES COORDINATES TO BE FROM CENTER OF IMAGE
  imageMode(CENTER);  
  //CREATE THE PLAYER
  //POSITION THEM IN THE CENTER OF THE SCREEN
  player1 = new InteractableImageObject(100, height/2, player1img);
  player2 = new InteractableImageObject(width - 100, height/2, player2img);
...
void CheckProjectileCollisionWithPlayers() {
  ...
  void CheckPlayerCollisionWithCircles() {
    for (int i = 0; i < circles.size(); i++) {
      ...
      if (player1.hitCircle(circles.get(i)) ) {
        //YES? TURN THE SCREEN RED.
        backgroundColor = color(255, 0, 0);
      }

      if (player2.hitCircle(circles.get(i)) ) {
        //YES? TURN THE SCREEN RED.
        backgroundColor = color(255, 0, 0);
      }
    }
